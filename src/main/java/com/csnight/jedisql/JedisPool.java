...
import com.csnight.jedisql.util.JedisURIHelper;
...
public class JedisPool extends JedisPoolAbstract {
    ...
    public JedisPool(final String host) {
        ...
        if (JedisURIHelper.isValid(uri)) {
            this.internalPool = new GenericObjectPool<JediSQL>(new JedisFactory(uri,
                    Protocol.DEFAULT_TIMEOUT, Protocol.DEFAULT_TIMEOUT, null), new GenericObjectPoolConfig());
        } else {
            this.internalPool = new GenericObjectPool<JediSQL>(new JedisFactory(host,
    ...
    public JedisPool(final GenericObjectPoolConfig poolConfig, final String host, int port,
                     ...
                     final SSLSocketFactory sslSocketFactory, final SSLParameters sslParameters,
                     final HostnameVerifier hostnameVerifier) {
        this(poolConfig, host, port, timeout, user, password, database, null, ssl, sslSocketFactory,
                sslParameters, hostnameVerifier);
    }
    ...
    public JedisPool(final GenericObjectPoolConfig poolConfig, final String host, final int port,
                     final int connectionTimeout, final int soTimeout, final String user,
    ...
    @Override
    public JediSQL getResource() {
        JediSQL jediSQL = super.getResource();
        jediSQL.setDataSource(this);
        return jediSQL;
    }

    @Override
    protected void returnBrokenResource(final JediSQL resource) {
        if (resource != null) {
