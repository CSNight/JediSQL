...
import com.csnight.jedisql.commands.*;
...
public class JediSQL extends BinaryJedis implements JedisCommands, MultiKeyCommands,
    ...
    public JediSQL(final String host) {
        super(host);
    }

    public JediSQL(final HostAndPort hp) {
        super(hp);
    }

    public JediSQL(final String host, final int port) {
        super(host, port);
    }

    public JediSQL(final String host, final int port, final boolean ssl) {
        super(host, port, ssl);
    }
    ...
    public JediSQL(final String host, final int port, final int timeout) {
        super(host, port, timeout);
    }

    public JediSQL(final String host, final int port, final int timeout, final boolean ssl) {
    ...
    public JediSQL(final String host, final int port, final int connectionTimeout, final int soTimeout) {
    ...
    public JediSQL(JedisShardInfo shardInfo) {
        super(shardInfo);
    }

    public JediSQL(URI uri) {
        super(uri);
    }

    public JediSQL(URI uri, final SSLSocketFactory sslSocketFactory, final SSLParameters sslParameters,
                   final HostnameVerifier hostnameVerifier) {
        super(uri, sslSocketFactory, sslParameters, hostnameVerifier);
    }

    public JediSQL(final URI uri, final int timeout) {
        super(uri, timeout);
    }

    public JediSQL(final URI uri, final int timeout, final SSLSocketFactory sslSocketFactory,
                   final SSLParameters sslParameters, final HostnameVerifier hostnameVerifier) {
        super(uri, timeout, sslSocketFactory, sslParameters, hostnameVerifier);
    }

    public JediSQL(final URI uri, final int connectionTimeout, final int soTimeout) {
    ...
    private JSONArray parseSqlReply(List<Object> res) {
                ...
                if (o instanceof List) {
                    ...
                    for (Object item : oo) {
                        if (item instanceof Long) {
                            ...
                            items.add(new String((byte[]) item));
                        }
                    }
                    jsonArray.add(items);
                    ...
                    jsonArray.add(new String((byte[]) o));
                }
            }
        } catch (Exception ex) {
        }
    ...
    public String create_db(String db, String path) {
        checkIsInMultiOrPipeline();
        client.create_db(db, path);
        return client.getBulkReply();
    }

    public List<Object> exec(String db, String sql) {
        checkIsInMultiOrPipeline();
        client.exec(db, sql);
        return client.getObjectMultiBulkReply();
    }

    public JSONArray exec(String db, String sql, boolean cast) {
        checkIsInMultiOrPipeline();
        client.exec(db, sql);
        return parseSqlReply(client.getObjectMultiBulkReply());
    }
