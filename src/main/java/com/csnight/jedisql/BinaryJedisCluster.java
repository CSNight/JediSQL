...
import com.csnight.jedisql.commands.BinaryJedisClusterCommands;
import com.csnight.jedisql.commands.JedisClusterBinaryScriptingCommands;
import com.csnight.jedisql.commands.MultiKeyBinaryJedisClusterCommands;
...
public class BinaryJedisCluster implements BinaryJedisClusterCommands,
    ...
    public Set<byte[]> hkeys(final byte[] key) {
        return new JedisClusterCommand<Set<byte[]>>(connectionHandler, maxAttempts) {
            @Override
            public Set<byte[]> execute(JediSQL connection) {
    ...
    public Map<byte[], byte[]> hgetAll(final byte[] key) {
        return new JedisClusterCommand<Map<byte[], byte[]>>(connectionHandler, maxAttempts) {
            @Override
            public Map<byte[], byte[]> execute(JediSQL connection) {
    ...
    public ScanResult<byte[]> scan(final byte[] cursor, final ScanParams params) {
        ...
        return new JedisClusterCommand<ScanResult<byte[]>>(connectionHandler, maxAttempts) {
            @Override
            public ScanResult<byte[]> execute(JediSQL connection) {
    ...
    public ScanResult<Map.Entry<byte[], byte[]>> hscan(final byte[] key, final byte[] cursor) {
        return new JedisClusterCommand<ScanResult<Map.Entry<byte[], byte[]>>>(connectionHandler,
                maxAttempts) {
            @Override
            public ScanResult<Map.Entry<byte[], byte[]>> execute(JediSQL connection) {
