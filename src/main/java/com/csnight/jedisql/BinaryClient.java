...
import com.csnight.jedisql.params.*;
...
import static com.csnight.jedisql.Protocol.Command.EXISTS;
import static com.csnight.jedisql.Protocol.Command.GET;
import static com.csnight.jedisql.Protocol.Command.KEYS;
import static com.csnight.jedisql.Protocol.Command.PING;
import static com.csnight.jedisql.Protocol.Command.PSUBSCRIBE;
import static com.csnight.jedisql.Protocol.Command.PUNSUBSCRIBE;
import static com.csnight.jedisql.Protocol.Command.SET;
import static com.csnight.jedisql.Protocol.Command.SUBSCRIBE;
import static com.csnight.jedisql.Protocol.Command.TIME;
import static com.csnight.jedisql.Protocol.Command.UNSUBSCRIBE;
import static com.csnight.jedisql.Protocol.Command.*;
import static com.csnight.jedisql.Protocol.Keyword.*;
import static com.csnight.jedisql.Protocol.RediSQLCommand;
...
public class BinaryClient extends Connection {
    ...
    public void create_db(byte[] db, byte[] path) {
        sendCommand(RediSQLCommand.CREATE_DB, db, path);
    }

    public void exec(byte[] db, byte[] sql) {
        sendCommand(RediSQLCommand.EXEC, db, sql);
    }

    public void exec_now(byte[] db, byte[] query) {
        sendCommand(RediSQLCommand.EXEC_NOW, db, query);
    }

    public void query(byte[] db, byte[] query) {
        sendCommand(RediSQLCommand.QUERY, db, query);
    }

    public void query_now(byte[] db, byte[] query) {
        sendCommand(RediSQLCommand.QUERY_NOW, db, query);
    }

    public void query_into(byte[] stream, byte[] db, byte[] query) {
        sendCommand(RediSQLCommand.QUERY_INTO, stream, db, query);
    }

    public void query_into_now(byte[] stream, byte[] db, byte[] query) {
        sendCommand(RediSQLCommand.QUERY_INTO_NOW, stream, db, query);
    }

    public void create_statement(byte[] db, byte[] stmt_name, byte[] stmt_query) {
        sendCommand(RediSQLCommand.CREATE_STATEMENT, db, stmt_name, stmt_query);
    }

    public void create_statement_now(byte[] db, byte[] stmt_name, byte[] stmt_query) {
        sendCommand(RediSQLCommand.CREATE_STATEMENT_NOW, db, stmt_name, stmt_query);
    }

    public void exec_statement(String... args) {
        sendCommand(RediSQLCommand.EXEC_STATEMENT, args);
    }

    public void exec_statement_now(String... args) {
        sendCommand(RediSQLCommand.EXEC_STATEMENT_NOW, args);
    }

    public void query_statement(String... args) {
        sendCommand(RediSQLCommand.QUERY_STATEMENT, args);
    }

    public void query_statement_now(String... args) {
        sendCommand(RediSQLCommand.QUERY_STATEMENT_NOW, args);
    }

    public void query_statement_into(String... args) {
        sendCommand(RediSQLCommand.QUERY_STATEMENT_INTO, args);
    }

    public void query_statement_into_now(String... args) {
        sendCommand(RediSQLCommand.QUERY_STATEMENT_INTO_NOW, args);
    }

    public void delete_statement(byte[] db, byte[] stmt_name) {
        sendCommand(RediSQLCommand.DELETE_STATEMENT, db, stmt_name);
    }

    public void delete_statement_now(byte[] db, byte[] stmt_name) {
        sendCommand(RediSQLCommand.DELETE_STATEMENT_NOW, db, stmt_name);
    }

    public void update_statement(byte[] db, byte[] stmt_name, byte[] stmt_query) {
        sendCommand(RediSQLCommand.UPDATE_STATEMENT, db, stmt_name, stmt_query);
    }

    public void update_statement_now(byte[] db, byte[] stmt_name, byte[] stmt_query) {
        sendCommand(RediSQLCommand.UPDATE_STATEMENT_NOW, db, stmt_name, stmt_query);
    }

    public void copy(byte[] db1, byte[] db2) {
        sendCommand(RediSQLCommand.COPY, db1, db2);
    }

    public void copy_now(byte[] db1, byte[] db2) {
        sendCommand(RediSQLCommand.COPY_NOW, db1, db2);
    }

    public void statistics() {
        sendCommand(RediSQLCommand.STATISTICS);
    }

    public void version() {
        sendCommand(RediSQLCommand.VERSION);
    }
}
